...
# =============================================================================
class Drevo:
    '''Razred dvojiško drevo'''

    def __init__(self, podatek=None, levo=None, desno=None):

        '''Metoda init shrani podatke'''

        if podatek is None:  # če je podatek none, imamo pa levvo in desno poddrevi

            self._koren = None

            if levo is not None or desno is not None:
                raise Exception('Koren ima podatek None, levo in desno poddrevo pa sta podatka')

        else:

            self._koren = (podatek, levo, desno)

        if levo == None or desno == None:
    ...
    def prazno(self):
        ...
        '''vrne, če je drevo prazno'''

        return self._prazno

    def podatek(self):

        '''vrne podatek v korenu'''

        if self.prazno():
            raise Exception('Ni podatka v prazno')
    ...
    def leviOtrok(self):
        ...
        if self.prazno():
            raise Exception("Ni levega otroka");
    ...
    def desniOtrok(self):
        ...
        if self.prazno():
            raise Exception("Ni desnega otroka");
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI5NCwidXNlciI6MTY2MH0:1iYpfR:rHalXwHM7gZL5EpIZ8R2kEYpJ5M'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI5NSwidXNlciI6MTY2MH0:1iYpfR:GT9EFnPH5j6KAE-5cGlY8Tl30dk'
        try:
