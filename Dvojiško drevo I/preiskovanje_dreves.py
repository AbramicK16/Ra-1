...
def vsota(drevo):
    '''vrne vsoto vseh števil v drevesu 'drevo' '''
    if drevo == None:
        return 0 #če to ni drevo, vrnemo 0
    if drevo.prazno():
        return 0 #v praznem drevesu je seštevek vseh elementov = 0
    return vsota(drevo.levoPoddrevo()) + vsota(drevo.desnoPoddrevo()) + drevo.vrniPodatek()
    #vrnemo vsoto v levem podrevesu + vsota v desnem poddrevesu + podatek v korenu
# =====================================================================@022282=
# 2. podnaloga
...
def stevilo_listov(drevo):
    '''vrne število listov v drevesu `drevo`'''
    if drevo == None:
        return 0
    if drevo.prazno(): #prazno drevo nima listov
        return 0
    if drevo.levoPoddrevo() == None and drevo.desnoPoddrevo() == None:
        return 1 #če (pod)drevo nima nobenega sinova je to list
    return stevilo_listov(drevo.levoPoddrevo()) + stevilo_listov(drevo.desnoPoddrevo())
    #če ima drevo enega ali več sinov to ni list in pogledamo globje
# =====================================================================@022283=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI4MSwidXNlciI6MTY2MH0:1iYphQ:RXUEnHBR22hL-vzyGoUOZH2Z0ow'
        try:
            test_data = [
                ('vsota(Drevo())', 0),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI4MiwidXNlciI6MTY2MH0:1iYphQ:vJdu1FI2YZHZUYuZkmdWv0wWsK0'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI4MywidXNlciI6MTY2MH0:1iYphQ:ebYFku-iuFWbh7PTF-Ld8_7NqiA'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjI4NCwidXNlciI6MTY2MH0:1iYphQ:e9Bm3LL-YCpTS1yRsFSj-JDIzro'
        try:
            test_data = [
