...
def nicelni_pari_v_urejenem(seznam):
    '''vrne True, če v urejenem seznamu celih števil obstaja par elementov,
    ki se sešteje v 0, sicer pa False'''
    stevila = set()
    for el in seznam:
        if el*(-1) in stevila:
            return True
...
def nicelni_pari_v_neurejenem(seznam):
    '''vrne True, če v neurejenem seznamu celih števil obstaja par elementov,
    ki se sešteje v 0, sicer vrne False'''
    stevila = set()
    for el in seznam:
        if el * (-1) in stevila:
            return True
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3MSwidXNlciI6MTY2MH0:1iaO0w:apq6rV5kVktE2UIpeWc9dK5p-Sc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3MiwidXNlciI6MTY2MH0:1iaO0w:_5VJBtA-6JaQ_jB7jWDU1XFtlWU'
        try:
