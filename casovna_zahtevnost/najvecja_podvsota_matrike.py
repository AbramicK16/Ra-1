...
def matrika_delnih_vsot(matrika):
    ...
    for i in range(len(matrika)):
        ...
        for j in range(len(matrika[0])):
            novaMatrika[a].append(0)
            if i > 0 and j > 0:
                novaMatrika[i][j] = novaMatrika[i][j - 1] + matrika[i][j] + novaMatrika[i - 1][j] - novaMatrika[i - 1][j - 1]
            elif i == 0 and j > 0:
                novaMatrika[i][j] = novaMatrika[i][j - 1] + matrika[i][j]
            else:
                novaMatrika[i][j] = novaMatrika[i - 1][j] + matrika[i][j]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3MywidXNlciI6MTY2MH0:1iaO7V:A5aSBrI6ARibkxI3XIZ8MUIPcNc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3NCwidXNlciI6MTY2MH0:1iaO7V:lT-hCV5gr2wiUZIbV9FZbKbspqo'
        try:
