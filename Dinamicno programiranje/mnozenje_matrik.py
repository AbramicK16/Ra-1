...
def stevilo_mnozenj(dim):
    '''Vrne najmanjse stevilo mnozenj realnih stevil, ki jih potrebujemo, da
    zmnozimo matrike danih velikosti'''

    def stolpci(i): return dim[i][1]
    def vrstice(i): return dim[i][0]
    ...
    for l in range(1, n + 1):
        for start in range(0, n - l + 1):
            ...
            f[(start, end)] = min(
                f[(start, mid)] +
                f[(mid + 1, end)] +
                vrstice(start) * stolpci(mid) * stolpci(end)
...
print(stevilo_mnozenj(([(5,7), (7,2), (2,12), (12,14)])))
...
#     '((A1(A2(A3A4)))A5)'
# =============================================================================
def vrstni_red_trojke(dim):
    '''Vrne vrstni red množenj matrik, pri katerem je potrebnih najmanj množenj realnih števil'''
    tabela = [dim[0][0]]
    for el in dim:
        tabela.append(el[1])
    ...
    m = [[0 for x in range(len(tabela))] for x in range(len(tabela))]
    n = len(tabela)
    ...
    for L in range(2, n):
        for i in range(1, n - L + 1):
            ...
            for k in range(i, j):
                ...
                q = m[i][k][0] + m[k + 1][j][0] + tabela[i - 1] * tabela[k] * tabela[j]
                if q < m[i][j][0]:
                    m[i][j] = (q, k)
                    slovarTrojk[i, j] = k
    return m, slovarTrojk
...
def vrstni_red(dim):
    return pomoc(vrstni_red_trojke(dim)[1], 1, len(dim), dim)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzM0MywidXNlciI6MTY2MH0:1ioSzd:iliqabhb5nKZMqmXS-_e1PVvT2w'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzM0NCwidXNlciI6MTY2MH0:1ioSzd:J51l7uTJlNnkn19sQBTfVtzzPaA'
        try:
