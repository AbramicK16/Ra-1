...
# referenco na prvi vozel v spremenjeni verigi vozlov. Če veriga vozlov
...
# =============================================================================
from veriga_vozlov import *


def vstavi_na_mesto(prvi, mesto, x):
    '''na "mesto" vstavi podatek x'''
    vozel = prvi
    if mesto == 0:
        return dodaj_na_zacetek(prvi, x)

    for i in range(mesto):
        if i != (mesto - 1):  # ce je slucajno na zadnjem mestu breakamo
            if vozel.naslednji is None:
                vozel.naslednji = Vozel(None)

        if i == (mesto - 1):
            vozel.naslednji = Vozel(x, vozel.naslednji)
            break
...
def vstavi_v_urejenega(prvi, x):
    '''vstavi številko, tako da bo seznam urejen'''
    if x <= prvi.podatek:
        v = Vozel(x, vozel)
        return v
    if prvi.naslednji is None:
        prvi.naslednji = Vozel(x, prvi.naslednji)
    else:
...
# referenco na vozel ter element `x`. Funkcija mora verigo popraviti,
# tako da odstrani vozle, ki kot podatek vsebujejo element `x`. A definicija
# vsebuje več napak. Odpravi jih. Funkcija naj vrne referenco na
...
from veriga_vozlov import Vozel, dodaj_na_konec, dodaj_na_zacetek, vrni_seznam, iz_seznama
def odstrani(v, x):
    '''odstrani vse podatke iz verige'''
    v = Vozel(0, v)
    vozel = v
    while vozel.naslednji != None:
        if vozel.naslednji.podatek == x:
            vozel.naslednji = vozel.naslednji.naslednji
        else:
            vozel = vozel.naslednji
    return v.naslednji
# =====================================================================@022100=
...
# je več napak. Odpravi jih. Funkcija naj vrne referenco
...
def multipliciraj_verigo(v, n):
    '''iz vsakega vozla naredi n zaporednih kopij'''
    if v is None:
        return None
    vozel = v
    while vozel.naslednji is not None:
        for i in range(n - 1):
            vozel.naslednji = Vozel(vozel, vozel.naslednji)
        vozel = vozel.naslednji
    return v
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA5N30:1iOid8:l1kr2jF9JVBwTuFKnAYVHWDE3mc'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA5OH0:1iOid8:IOiP79g6q24OCBqTMy-ShviujLw'
        try:
            test_data = [
                (["v = iz_seznama([1, 2, 4, 7, 8])",
                  "v = vstavi_v_urejenega(v, 3)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA5OX0:1iOid8:SG1t8HWdUwMynXgX01K0tApKq6U'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 5, 3, 5])",
                  "v = odstrani(v, 5)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjEwMH0:1iOid8:0vHVtZfukHd7TAUWc-Kh65YcKWk'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2])",
                  "v = multipliciraj_verigo(v, 3)",
