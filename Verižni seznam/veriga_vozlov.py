...
def vrni_seznam(prvi):
    '''vrne seznam vseh podatkov v verigi vozlov, ki se prične
    z vozlom prvi'''
    seznam = list()
    vozel = prvi
    while vozel is not None:
        seznam.append(vozel.podatek)
...
def dodaj_na_zacetek(prvi, x):
    '''na začetek verige vozlov, ki se začne z vozlom prvi,
    doda nov vozel '''
    vozel = prvi #prvi vozel v verigi
    novZacetek = Vozel(x, prvi) #verigi dodam nov vozel 'x' na zacetek, prvi bo tedaj naslednji
    return novZacetek
# =====================================================================@022039=
# 3. podnaloga
...
def dodaj_na_konec(prvi,x):
    '''na konec verige vozlov, ki se začne z vozlom prvi,
    doda nov vozel, ter vrne referenco na prvi vozel v verigi'''
    vozel = prvi
    if prvi is None: #pogledamo, če prvi kaže na none
        return Vozel(x)
...
def iz_seznama(seznam):
    '''sestavi novo verigo, ki kot podatke v vozlih zaporedoma vsebuje
    elemente seznama seznam'''
    veriga = Vozel(-1)
    vozel = veriga
    for el in seznam:
        vozel.naslednji = Vozel(el)
        vozel = vozel.naslednji
    return veriga.naslednji
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjAzN30:1iNIJ3:hi9s0EMPVOeUSlb5jvwzPW9rZXE'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjAzOH0:1iNIJ3:-_8OeOF49e-OTPm8HgGVigwmpJM'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjAzOX0:1iNIJ3:ZY4iT3ViUZgLSc-Ab86HKW3YpJE'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA0MH0:1iNIJ3:Hwdf9aDyCLAmWg4ZekOEA5fcgLw'
        try:
            test_data = [
                (["v = iz_seznama([])",
                  "l = vrni_seznam(v)"],
