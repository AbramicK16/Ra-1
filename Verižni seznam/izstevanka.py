...
# iz kroga, tako da zbrišemo $n$-tega in začnemo ponovno izštevanje pri
# naslednjemu. Funkcija na vrne ime otroka, ki bo mižal. Zgled:
# 
#     >>> kdo_mizi(['Mojca', 'Janez', 'Micka', 'Peter', 'Matjaž', 'Jožefa'], 5)
#     'Mojca'
# =============================================================================
#import vozel from Vozel
def kdo_mizi(imena,n):
    '''najprej sestavi krožni seznam, nato pa vozle izločamo iz kroga, tako
    da zbrišemo n-tega in začnemo ponovno izštevanje pri naslednjemu'''
    ind = 0
    while len(imena) > 1:
        ind = (ind + n - 1) % len(imena)
        del imena[ind]
    return imena[0]
# =====================================================================@022174=
# 2. podnaloga
...
def zgodovina_izstevanja(imena, n):
    '''vrne seznam, ki vsebuje imena v takem vrstnem redu kot smo jih
    izločali pri izštevanki'''
    ind = 0
    seznam = list()
    while len(imena) > 0:
        ind = (ind + n - 1) % len(imena)
        seznam.append(imena[ind])
        del imena[ind]
    return seznam
# =====================================================================@022175=
# 3. podnaloga
...
# =============================================================================
from veriga_vozlov import Vozel, dodaj_na_konec, dodaj_na_zacetek, iz_seznama, vrni_seznam
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjE3M30:1iNIee:-By--OZfDT65hBF3Rn5oyWYRESQ'
        try:
            test_data = [
                ("kdo_mizi(['Mojca', 'Janez', 'Micka', 'Peter', 'Matjaž', 'Jožefa'], 5)", 'Mojca'),      
            ]
            for td in test_data:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjE3NH0:1iNIee:OZG3B1vkgRDfNH8F7FcNpIUf5lc'
        try:
            test_data = [
                ("zgodovina_izstevanja(['Mojca', 'Janez', 'Micka', 'Peter', 'Matjaž', 'Jožefa'], 5)",
                 ['Matjaž', 'Peter', 'Jožefa', 'Janez', 'Micka', 'Mojca']),      
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjE3NX0:1iNIee:RV3-KfB5z6AXIrC27SKvEMYC6UM'
        try:
            test_data = [
                (["v = odhajanje_iz_kroga(['Mojca', 'Janez', 'Micka', 'Peter', 'Matjaž', 'Jožefa'], 5)",
                  "l = v.vrni_seznam()"],
                 {'l': ['Mojca', 'Micka', 'Janez', 'Jožefa', 'Peter', 'Matjaž']}),
            ]
            for td in test_data:
                if not Check.run(*td):
                    break
