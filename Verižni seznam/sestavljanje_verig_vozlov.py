...
def stevilska_veriga(a, b):
    '''vrne referenco na prvi vozel, ki zaporedoma vsebuje števila od a do b'''
    vozel = Vozel(a)
    a1 = a + 1
    while a1 <= b:
        dodaj_na_konec(vozel, a1)
        a1 += 1
    return vozel
# =====================================================================@022047=
# 2. podnaloga
...
def sodi_in_lihi(v):
    '''vrne dve verigi, ki ju dobi tako, da v eno naloži vozle, ki
    vsebujejo sode podatke, v drugo pa vozle, ki vsebujejo lihe podatke.
    Funkcija vrne par, in sicer referenci na začetna vozla v obeh verigah'''
    if v is None:
        return None, None
...
def podvoji_verigo(prvi):
    '''vrne novo, identično kopijo verige'''
    kopija = None
    vozel = None
    if prvi:
        while prvi is not None:
            if kopija is None:
                kopija = Vozel(prvi.podatek)
                vozel = kopija
            else:
                vozel.naslednji = Vozel(prvi.podatek)
...
def stakni_verigi(v1, v2):
    '''vrne novo verigo, tako da stakne dve drugi verigi'''
    veriga = podvoji_verigo(v1)
    if v2:
        while v2 is not None:
            veriga = dodaj_na_konec(veriga, v2.podatek)
            v2 = v2.naslednji
    return veriga
# =====================================================================@022050=
# 5. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA0Nn0:1iNIPd:MCZxwUnpLYlY3esGY2NmmqmfOXg'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA0N30:1iNIPd:GS67NHgJbcv9BPC7GJJ9Mad6IQ8'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v1, v2 = sodi_in_lihi(v)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA0OH0:1iNIPd:v9xVTrRKFz333hA8CQpThjWDGUE'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA0OX0:1iNIPd:rbPjbAqpa7h0HMgbQ6_o3Glbezs'
        try:
            test_data = [
                (["v1 = iz_seznama([1, 2, 3])",
                  "v2 = iz_seznama([4, 5, 6])",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA1MH0:1iNIPd:tcuu1XxtJOOOoa83t1upGjJu9jU'
        try:
            test_data = [
                (["v1 = iz_seznama([7, 5, 2])",
                  "v2 = iz_seznama([1, 3, 4, 9, 8])",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoxNjYwLCJwYXJ0IjoyMjA1MX0:1iNIPd:Zs1P8-43Bm_LjllfNgL6l4BoqOc'
        try:
            test_data = [
                (["v = iz_seznama([7, 5, 2, 1, 3, 4, 9, 8])",
                  "v1, v2 = odpni_zadrgo(v)",
