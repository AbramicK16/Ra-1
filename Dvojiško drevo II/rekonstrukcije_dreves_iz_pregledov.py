...
def drevo_vmesni_premi(vmesni, premi):
    '''Funkcija sprejme vmesni in premi pregled dvojiškega drevesa brez ponovljenih elementov
    in vrne dvojiško drevo.'''
    if vmesni == [] or premi == []: #če je kateri od seznamov prazen, vrnemo prazno drevo
        return Drevo()
    #rekurzivni klic na desnem in levem poddrevesu
    return Drevo(premi[0],
        levo = drevo_vmesni_premi(vmesni[:vmesni.index(premi[0])],
            premi[1:(vmesni.index(premi[0])+1)]),
                 desno = drevo_vmesni_premi(vmesni[(vmesni.index(premi[0])+1):],
                    premi[(vmesni.index(premi[0])+1):]))
# =====================================================================@022410=
# 2. podnaloga
...
def drevo_vmesni_obratni(vmesni, obratni):
    '''Funkcija sprejme vmesni in obratni pregled dvojiškega drevesa brez ponovljenih elementov
    in vrne dvojiško drevo.'''
    if vmesni == [] or obratni == []: #če je pregled prazen vrnemo prazno drevo
        return Drevo()
    return Drevo(obratni[-1],
        levo = drevo_vmesni_obratni(vmesni[:vmesni.index(obratni[-1])],
            obratni[:vmesni.index(obratni[-1])]),
                desno = drevo_vmesni_obratni(vmesni[(vmesni.index(obratni[-1])+1):],
                    obratni[vmesni.index(obratni[-1]):-1]))
# =====================================================================@022411=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQwOSwidXNlciI6MTY2MH0:1iYpkX:XP726EQdZluCjbgRqomXf2oywb4'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQxMCwidXNlciI6MTY2MH0:1iYpkX:m6aPYKpzKUCQA3JTCkPXbfOXy-I'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQxMSwidXNlciI6MTY2MH0:1iYpkX:9-QAKB5g12Mty00tGDZdNKAkFww'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQxMiwidXNlciI6MTY2MH0:1iYpkX:i2DjdMrNG848oM-XvTUoQ96Bxmc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjQxMywidXNlciI6MTY2MH0:1iYpkX:Pgru1frKGGOmOXs2PpIUAF8PYQc'
        try:
